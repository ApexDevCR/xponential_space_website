...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function handleAllScroll() {
    ...
    if (p3 > 0) {
      ...
      /* ── Single value + Year: animate together from 2025 to 2035 ── */
      const cp = clamp01((rawP - 0.52) / (0.88 - 0.52));
      const ce = easeOut3(cp);

      // Single value: $630B (2025) → $1.8T (2035)
      if (globeEls.valMain) {
        const val = lerp(630, 1800, ce);
        if (val < 1000) {
          globeEls.valMain.textContent = "$" + Math.round(val) + "B";
        } else {
          globeEls.valMain.textContent = "$" + (val / 1000).toFixed(1) + "T";
        }
      }

      /* ── Year counter: starts alongside value ── */
      const yearEl = document.querySelector(".stat-year");
      if (yearEl) {
        yearEl.textContent = Math.round(lerp(2025, 2035, ce));
      }
    } else {
      /* ── Reset everything when scrolled back above ── */
      if (globeEls.curve) {
        globeEls.curve.style.opacity = 0;
        globeEls.curve.style.visibility = "hidden";
        // Reset to 100% top clip (hidden from the top)
